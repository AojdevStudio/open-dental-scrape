#!/usr/bin/env python3
"""
process_manual_docs_fixed.py - Process OpenDental manual documentation from JSON
Adjusted to handle the actual JSON structure with 'header' types and 'level' values
"""
...
def extract_sections(manual_content):
    ...
    # Process the main Manual content
    for item in manual_content.get("Manual", []):
        if item["type"] == "header" and item.get("level") == 1:
        ...
        elif item["type"] == "header" and item.get("level") == 2:
        ...
        elif item["type"] == "header" and item.get("level") == 3:
    ...
    # Add the last section from Manual
    if current_section and current_content:
        sections.append({
            "section": current_section,
            "subsection": current_subsection,
    ...
    # Reset for Module content
    current_section = "Module"
    current_subsection = None
    current_topic = None
    current_content = []
    
    # Process Module content if it exists
    for item in manual_content.get("Module", []):
        if item["type"] == "header" and item.get("level") == 1:
            # Save previous section if exists
            if current_section and current_content:
                sections.append({
            ...
            current_section = "Module - " + item["text"]
            current_subsection = None
            current_topic = None
            current_content = []
    ...
    # Add the last section from Module
    if current_section and current_content:
        sections.append({
            "section": current_section,
            "subsection": current_subsection,
...
def process_section_content(content):
    ...
    for item in content:
        ...
        if content_type == "paragraph":
            ...
            # For nested headers within content
            level = item.get("level", 4)  # Default to level 4 if not specified
            processed_content.append({
                "type": "heading",
                "level": level,
                "content": item.get("text", "")
...
def create_doc_id(section, subsection, topic):
    ...
    if section:
        ...
        components.append(section_slug)
    if subsection:
        subsection_slug = subsection.lower().replace(" ", "-").replace("/", "-")
        components.append(subsection_slug)
    if topic:
        topic_slug = topic.lower().replace(" ", "-").replace("/", "-")
        components.append(topic_slug)
...
def process_manual_docs(input_file, output_dir):
    ...
    for section in sections:
        ...
        # Skip sections with no processed content
        if not section["processed_content"]:
            continue
